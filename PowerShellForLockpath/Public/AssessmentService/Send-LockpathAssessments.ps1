function Send-LockpathAssessments {
    ...
    begin {
        Write-LockpathInvocationLog -Service AssessmentService
    }
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess("Issuing Assessment with attributes: $([environment]::NewLine) $($params.Body)", "attributes $($params.Body)", 'Issuing Assessment with attributes:')) {
            ...
            Write-LockpathLog -Message "$($PSCmdlet.CommandRuntime.ToString()) ShouldProcess confirmation was denied." -Level Verbose -Service AssessmentService
        }
    }
