# Assumes the debug script has the same name basename as the module
$moduleName = ($myInvocation.MyCommand.Name).Split('.')[0]

# Get public functions assumed structure is .\src\public
$publicFunctions = Get-ChildItem -Recurse -Path .\src\public\ | Where-Object { ! $_.PSIsContainer } | ForEach-Object -Process { [System.IO.Path]::GetFileNameWithoutExtension($_) }
...
# import the output module into the current session
Import-Module "$buildPath\$moduleName.psd1" -Force
